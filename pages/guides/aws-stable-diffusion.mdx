---
title: Stable Diffusing on AWS
date: 2023/05/25
description: Running Automatic1111 on AWS can be hard. It took me days to figure out all the nitty gritty details. This guide is a step-by-step walkthrough of the process from configuring an EC2 instance to running the WebUI.
---
...
2. Search for `Running On-Demand G and VT instances` and click on it.
3. Click on `Request Quota Increase` and enter the value `4` into the input box.
4. Click "Request" to submit your quota increase request.
...
Install themes, extensions, embeddings, models and ControlNet. These are all **optional**.
...
Fish shell function to easily start, stop and view the status of the EC2 instance. Replace `<instance-id>` with the ID of your EC2 instance. This requires the [AWS CLI](https://aws.amazon.com/cli/) to be installed and configured.
...
This configuration was inspired by [mikeage/stable-diffusion-aws](https://github.com/mikeage/stable-diffusion-aws).
