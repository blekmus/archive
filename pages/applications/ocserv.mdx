...
---
import {Error} from '@components/callouts'


<Error>
  Ocserv __cannot run__ concurrently with nginx on port `443`. Either nginx should only take up port `80` or it should be disabled. Ocserv doesn't work behind a reverse proxy.
</Error>
