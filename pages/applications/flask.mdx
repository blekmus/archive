...
First create a separate database and user for the web-app in the [usual way](/general/database#creating). Then reference the db from the flask app. To get MariaDB working on SQLAlchemy you'll also need to install the `pymysql` package[^2].
...
When deploying, create a systemd service for the `gunicorn`. See more info about creating systemd services [here](/general/systemd/#service-files). 
...
Create a webhook instance json file and connect it with Github to listen to push requests from the repo. More info [here](/applications/webhook/#setup).

Then create a [new group](/general/global/#groups) for the flask app and couple it with the already existing `webhook` user created when setting up the webhook application. In it [whitelist](/general/global/#whitelisting) the `systemctl` commands for dealing with the gunicorn service.

Write a systemd service file with the `webhook` user and the new group for the hook instance. Find out how [here](/applications/webhook/#systemd-service).

## Integration Script

Since systemd runs services in isolated environments. Without access to the user shell nor any environment variables. So, for private repos an ssh-agent needs to be started every time the script is run for Github authentication. More info [here](/general/git/#automated-deployment).

First clean the repo of any untracked changes. Then pull the latest commit. Find how [here](/general/git/#userful-commands). Finally reload the site. Make sure to only use the `systemctl` commands that were defined in the group the hook instance is running on. 
